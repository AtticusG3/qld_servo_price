...
- Keep poll frequency internal (`DEFAULT_UPDATE_INTERVAL_HOURS` = 6 in `const.py`); do not expose update interval in config flow or options.
...
- Legacy stored `scan_interval` in config entry data/options is ignored; all entries poll on the fixed 6-hour interval.
- Unit test harness: `tests/components/qld_servo_price/test_loaders.py` provides `register_real_util_module()` for importlib-based loaders.
- Integration brand assets require Home Assistant 2026.3+ (see README); hassfest runs via `.github/workflows/hassfest.yml`.
